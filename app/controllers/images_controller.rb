class ImagesController < ApplicationController
  ...
  def show
    ...
    raise ActionController::RoutingError.new('Not Found') unless Image.exist?(params[:resource])

    # If there's a GIF, select it (since it could have multiple frames);
    # otherwise, pick the first image.
    if Image.exists?(params[:resource], 'gif')
      source = Image.first(params[:resource], 'gif')
    else
      source = Image.first(params[:resource])
    end
    ...
    @image = Image.first(params[:resource], params[:format], frame)

    if @image.nil? # We need to generate the image
      if source_file.length > 1
        path = Image.root.join(params[:resource])
